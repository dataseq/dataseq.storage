...
/**
 * Create a new Value.
 * @param {(ValueModel|ValueRecord)} data - A data object to create a new Value Db record.
 * @return {ValueRecord} Returns created ValueRecord object.
 */
ControlService.prototype.createValue = function(data) {
	try {
	...
	var params = {};
	params.ConditionExpression = '#key <> :key AND #range <> :range';
	...
	return models.Value.createAsync(data, params).then(get);
};
 ...
 * @return {ValueRecord} Returns db ValueRecord object.
 */
ControlService.prototype.putValue = function(data, options) {
	...
	return models.Value.createAsync(data, options && options.params).then(get);
};
